...
export const defaultCloudfrontFunction = (scope: Construct): cloudfront.Function => {
  ...
  return new cloudfront.Function(scope, 'SetHttpSecurityHeaders', {
    ...
    code: cloudfront.FunctionCode.fromInline(
      ...
      headers['content-security-policy'] = { value: \"default-src 'none'; img-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; manifest-src 'self'; connect-src 'self'; object-src 'none'\"}; \
